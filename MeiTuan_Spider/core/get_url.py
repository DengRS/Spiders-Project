...
class ConstructURL(object):
    ...
    def __str__(self):
        base_url = f'https://bj.meituan.com/meishi/api/poi/getPoiList?cityName={self.city}&uuid={self.cookie}&platform=1&partner=126&riskLevel=1&optimusCode=1&'
        sign = self.get_sign
        self.sign = self.encrypt.encrypt(sign)
        self.token = self.get_token(self.get_time, self.sign)
        return base_url + urlencode({'_token': self.encrypt.encrypt(self.token)})
